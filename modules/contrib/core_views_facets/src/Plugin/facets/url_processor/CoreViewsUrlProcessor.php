  ...
  public function setActiveItems(FacetInterface $facet) {
    ...
    switch (TRUE) {
      case $source instanceof CoreViewsExposedFilter:
        ...
        if ($url_parameters->has($views_filter_parameter)) {
          if ($views_filter['expose']['multiple']) {
            foreach ($url_parameters->get($views_filter_parameter, [], TRUE) as $value) {
              $facet->setActiveItem(trim($value, '"'));
            }
          }
          else {
            $value = $url_parameters->get($views_filter_parameter, NULL, TRUE);
